...
class TestUserData(TestCase):
    ...
    def test_benchmarks(self):
        ...
        with open(testfile, 'r') as fd:
            read_benchmarks = nuxhash.settings.read_benchmarks_from_file(fd, self.devices)
        device = self.devices[0]
        self.assertEqual(self.benchmarks, read_benchmarks)

    def test_persistent_settings(self):
        nuxhash.settings.save_persistent_data(self.testdir,
                                              self.settings, self.benchmarks)
        read_settings, read_benchmarks = nuxhash.settings.load_persistent_data(self.testdir,
                                                                               self.devices)
        self.assertEqual(self.settings, read_settings)
